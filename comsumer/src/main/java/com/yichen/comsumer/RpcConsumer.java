...
import com.yichen.handler.RpcResponseHandler;
...
public class RpcConsumer {
    ...
    public void sendRequest(MiniProtocol<MiniRequest> protocol, RegisterService registryService) throws Exception {
        MiniRequest request = protocol.getBody();
        Object[] params = request.getParams();
        String serviceKey = RpcServiceHelper.buildServiceKey(request.getClassName(), request.getServiceVersion());

        int invokerHashCode = params.length > 0 ? params[0].hashCode() : serviceKey.hashCode();
        // 找到注册中心中 满足 hashcode 之后的服务器 源节点 信息
